...
function CouponPanel({ coupons, referenceTime }: { coupons: MemberCoupon[]; referenceTime: number }) {
  return <section className="mypage-panel"><PanelTitle eyebrow="MY BENEFITS" title="보유 쿠폰" description="사용 가능한 쿠폰과 사용 내역을 한눈에 확인하세요." />{coupons.length ? <div className="coupon-list">{coupons.map((coupon) => { const expired = Boolean(coupon.expiresAt && new Date(coupon.expiresAt).getTime() <= referenceTime); const unavailable = coupon.used || expired; return <article className={unavailable ? "is-used" : ""} key={coupon.id}><div><TicketPercent /><span><em>{coupon.used ? "USED" : expired ? "EXPIRED" : "AVAILABLE"}</em><strong>{coupon.name}</strong><small>쿠폰코드 {coupon.code}</small></span></div><b>{coupon.discountType === "percent" ? `${coupon.value}%` : formatPrice(coupon.value)}</b><footer><span>{coupon.minimumPurchase ? `${formatPrice(coupon.minimumPurchase)} 이상 구매 시` : "최소 결제금액 없음"}</span><span>{coupon.expiresAt ? `${new Date(coupon.expiresAt).toLocaleDateString("ko-KR")}까지` : "유효기간 제한 없음"}</span></footer></article>; })}</div> : <div className="mypage-empty"><TicketPercent /><h3>보유한 쿠폰이 없습니다.</h3><p>새로운 혜택이 발급되면 이곳에서 확인할 수 있습니다.</p></div>}</section>;
}
