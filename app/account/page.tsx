...
type AccountTab = "overview" | "orders" | "address" | "coupons" | "profile";
...
const tabs: Array<{ id: AccountTab; label: string; icon: typeof UserRound }> = [
  ...
  { id: "coupons", label: "쿠폰", icon: TicketPercent },
  { id: "profile", label: "회원·보안 정보", icon: KeyRound },
];
...
export default function AccountPage() {
  ...
  const verifyIdentity = async (password?: string) => {
    setBusy(true); setError(""); setNotice("");
    try { await auth.verifyIdentity(password); setNotice("본인 확인이 완료되었습니다."); }
    catch (nextError) { setError(firebaseErrorMessage(nextError)); throw nextError; }
    finally { setBusy(false); }
  };
    ...
    <main id="content" className="inner-page utility-page account-page">
      ...
      <section className="mypage-shell">
        ...
        <div className="mypage-content">
          {activeTab === "overview" && <Overview name={auth.profile?.displayName ?? "엘란 고객"} orderCount={auth.orders.length} addressCount={auth.profile?.addresses.length ?? 0} coupons={auth.profile?.coupons ?? []} referenceTime={couponReferenceTime} go={setActiveTab} />}
          ...
          {activeTab === "coupons" && <CouponPanel coupons={auth.profile?.coupons ?? []} referenceTime={couponReferenceTime} />}
          {activeTab === "profile" && <AccountSettingsPanel initialName={auth.profile?.displayName ?? ""} initialPhone={auth.profile?.phone ?? ""} email={auth.user.email ?? ""} isPasswordUser={auth.user.providerData.some((provider) => provider.providerId === "password")} busy={busy} notice={notice} error={error} verify={verifyIdentity} saveProfile={(displayName, phone) => run(() => auth.saveProfile({ displayName, phone }), "회원 정보를 변경했습니다.")} changePassword={(current, next) => run(() => auth.changePassword(current, next), "비밀번호를 변경했습니다.")} />}
        </div>
      </section>
    </main>
  );
...
function Overview({ name, orderCount, addressCount, coupons, referenceTime, go }: { name: string; orderCount: number; addressCount: number; coupons: MemberCoupon[]; referenceTime: number; go: (tab: AccountTab) => void }) { const availableCoupons = coupons.filter((coupon) => !coupon.used && (!coupon.expiresAt || new Date(coupon.expiresAt).getTime() > referenceTime)); const availableCoupon = availableCoupons[0]; return <section className="mypage-overview"><div className="dashboard-welcome"><p>WELCOME BACK</p><h2>안녕하세요, {name}님.</h2><span>당신을 위한 새로운 에디트가 준비되어 있습니다.</span></div>{availableCoupon && <div className="welcome-coupon-banner"><div><TicketPercent /><span><em>WELCOME GIFT</em><strong>{availableCoupon.name}</strong><small>{availableCoupon.expiresAt ? `${new Date(availableCoupon.expiresAt).toLocaleDateString("ko-KR")}까지` : "유효기간 제한 없음"} · 결제금액의 {availableCoupon.value}% 할인</small></span></div><b>{availableCoupon.value}%</b><button type="button" onClick={() => go("coupons")}>쿠폰 보기</button></div>}<div className="account-status-grid account-essential-grid"><button type="button" onClick={() => go("coupons")}><TicketPercent /><strong>{availableCoupons.length}</strong><span>사용 가능 쿠폰</span></button><button type="button" onClick={() => go("orders")}><ReceiptText /><strong>{orderCount}</strong><span>전체 주문</span></button><button type="button" onClick={() => go("address")}><MapPin /><strong>{addressCount}</strong><span>등록 배송지</span></button></div><div className="overview-cards"><button type="button" onClick={() => go("orders")}><span>ORDER HISTORY</span><strong>{orderCount}개의 주문</strong><p>주문 내역과 배송 위치를 확인하세요.</p></button><button type="button" onClick={() => go("address")}><span>DELIVERY ADDRESS</span><strong>배송지 {addressCount}개 관리</strong><p>받는 분과 주소를 변경할 수 있어요.</p></button><button type="button" onClick={() => go("profile")}><span>MEMBER &amp; SECURITY</span><strong>회원·보안 정보</strong><p>본인 확인 후 정보를 안전하게 변경하세요.</p></button></div></section>; }
...
function AddressPanel({ addresses, busy, notice, error, submit, remove, makeDefault }: { addresses: MemberAddress[]; busy: boolean; notice: string; error: string; submit: (data: MemberAddress) => void; remove: (addressId: string) => void; makeDefault: (addressId: string) => void }) {
  ...
  return <section className="mypage-panel"><div className="address-panel-heading"><PanelTitle eyebrow="DELIVERY" title="배송지 관리" description="집, 회사 등 자주 사용하는 배송지를 저장하고 기본 배송지를 선택하세요." /><button className="address-add-button" type="button" onClick={() => openEditor()}><Plus />새 배송지 추가</button></div>
    {addresses.length > 0 ? <div className="address-book-grid">{addresses.map((address) => {
      ...
      return <article className={`address-card${address.isDefault ? " is-default" : ""}`} key={address.id}><header><span><AddressIcon />{address.label || "배송지"}</span>{address.isDefault && <b>기본 배송지</b>}</header><div className="address-card-recipient"><strong>{address.recipient}</strong><span>{address.phone}</span></div><p><em>[{address.postalCode}]</em>{address.addressLine1}<br />{address.addressLine2}</p><footer>{!address.isDefault && <button className="address-default-button" type="button" disabled={busy} onClick={() => address.id && makeDefault(address.id)}>기본 배송지로 설정</button>}<div className="address-card-actions"><button type="button" aria-label={`${address.label} 배송지 수정`} onClick={() => openEditor(address)}><Pencil />수정</button><button className="address-delete" type="button" aria-label={`${address.label} 배송지 삭제`} disabled={busy} onClick={() => { if (address.id && window.confirm("이 배송지를 삭제할까요?")) remove(address.id); }}><Trash2 />삭제</button></div></footer></article>;
...
function CouponPanel({ coupons, referenceTime }: { coupons: MemberCoupon[]; referenceTime: number }) {
  const visibleCoupons = coupons.filter((coupon) => !coupon.used);
  return <section className="mypage-panel"><PanelTitle eyebrow="MY BENEFITS" title="보유 쿠폰" description="현재 보유한 쿠폰과 사용 기한을 한눈에 확인하세요." />{visibleCoupons.length ? <div className="coupon-list">{visibleCoupons.map((coupon) => { const expired = Boolean(coupon.expiresAt && new Date(coupon.expiresAt).getTime() <= referenceTime); return <article className={expired ? "is-used" : ""} key={coupon.id}><div><TicketPercent /><span><em>{expired ? "EXPIRED" : "AVAILABLE"}</em><strong>{coupon.name}</strong><small>쿠폰코드 {coupon.code}</small></span></div><b>{coupon.discountType === "percent" ? `${coupon.value}%` : formatPrice(coupon.value)}</b><footer><span>{coupon.minimumPurchase ? `${formatPrice(coupon.minimumPurchase)} 이상 구매 시` : "최소 결제금액 없음"}</span><span>{coupon.expiresAt ? `${new Date(coupon.expiresAt).toLocaleDateString("ko-KR")}까지` : "유효기간 제한 없음"}</span></footer></article>; })}</div> : <div className="mypage-empty"><TicketPercent /><h3>사용 가능한 쿠폰이 없습니다.</h3><p>새로운 혜택이 발급되면 이곳에서 확인할 수 있습니다.</p></div>}</section>;
}

function AccountSettingsPanel({ initialName, initialPhone, email, isPasswordUser, busy, notice, error, verify, saveProfile, changePassword }: { initialName: string; initialPhone: string; email: string; isPasswordUser: boolean; busy: boolean; notice: string; error: string; verify: (password?: string) => Promise<void>; saveProfile: (name: string, phone: string) => void; changePassword: (current: string, next: string) => void }) {
  const [verified, setVerified] = useState(false);
  const [verifyPassword, setVerifyPassword] = useState("");
  const confirmIdentity = async () => {
    try {
      await verify(verifyPassword);
      setVerified(true);
    } catch { /* run() renders the account error */ }
  };
  return <section className="mypage-panel"><PanelTitle eyebrow="MEMBER & SECURITY" title="회원·보안 정보" description="개인정보 보호를 위해 본인 확인 후 정보를 표시하고 변경합니다." />{!verified ? <div className="profile-verification"><ShieldCheck /><h3>본인 확인이 필요합니다.</h3><p>{isPasswordUser ? "현재 비밀번호를 한 번 더 입력해 주세요." : "Google 계정으로 한 번 더 확인해 주세요."}</p>{isPasswordUser && <label>현재 비밀번호<input value={verifyPassword} onChange={(event) => setVerifyPassword(event.target.value)} type="password" minLength={6} autoComplete="current-password" /></label>}<FormFeedback error={error} notice={notice} /><button className="primary-button" disabled={busy || (isPasswordUser && verifyPassword.length < 6)} type="button" onClick={confirmIdentity}>{busy ? "확인 중..." : isPasswordUser ? "비밀번호 확인" : "Google로 본인 확인"}</button></div> : <div className="account-settings-grid"><form className="mypage-form" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); saveProfile(String(data.get("displayName")), String(data.get("phone"))); }}><h3>회원 정보 변경</h3><div className="form-grid"><label>이름<input name="displayName" required defaultValue={initialName} /></label><label>연락처<input name="phone" required type="tel" defaultValue={initialPhone} /></label><label className="full">이메일<input value={email} disabled readOnly /></label></div><p className="form-help"><Mail />이메일 변경은 고객 지원을 통해 본인 확인 후 가능합니다.</p><button className="primary-button" disabled={busy} type="submit">회원 정보 저장</button></form>{isPasswordUser ? <form className="mypage-form" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); const next = String(data.get("next")); if (next !== String(data.get("confirm"))) return; changePassword(String(data.get("current")), next); event.currentTarget.reset(); }}><h3>비밀번호 변경</h3><label>현재 비밀번호<input name="current" required type="password" minLength={6} autoComplete="current-password" /></label><label>새 비밀번호<input name="next" required type="password" minLength={6} autoComplete="new-password" /></label><label>새 비밀번호 확인<input name="confirm" required type="password" minLength={6} autoComplete="new-password" /></label><button className="secondary-button" disabled={busy} type="submit">비밀번호 변경</button></form> : <div className="social-security"><ShieldCheck /><h3>Google 계정으로 로그인 중입니다.</h3><p>비밀번호는 Google 계정의 보안 설정에서 변경해 주세요.</p></div>}<FormFeedback error={error} notice={notice} /></div>}</section>;
}
